...
/*
 * package com.qobox.desktop.screens;
 * 
 ...
 * 
 * import com.j256.simplemagic.ContentInfo; import
 * com.j256.simplemagic.ContentInfoUtil; import
 * com.j256.simplemagic.ContentType;
 * 
 * import org.apache.commons.codec.binary.Base64; import
 * com.testautomationguru.utility.PDFUtil; import javax.mail.Flags; import
 * javax.mail.Folder; import javax.mail.Message; import
 * javax.mail.MessagingException; import javax.mail.Multipart; import
 * javax.mail.Part; import javax.mail.Session; import javax.mail.Store; import
 * javax.mail.internet.MimeBodyPart; import javax.mail.search.FlagTerm;
 * 
 * public class TestMail{ public TestMail(String username, String password,
 * String server, EmailsFolder inbox) throws Exception { Properties props =
 * System.getProperties(); props.setProperty("mail.store.protocol", "imap");
 * props.setProperty("mail.imap.ssl.enable", "true");
 * props.setProperty("mail.imaps.partialfetch", "false");
 * props.put("mail.mime.base64.ignoreerrors", "true");
 * 
 * Session mailSession = Session.getInstance(props); mailSession.setDebug(true);
 ...
 * 
 * for (Message mail : messages) { if (!mail.isSet(Flags.Flag.SEEN)) {
 * 
 * System.out.println("***************************************************");
 ...
 * 
 * // suppose 'message' is an object of type Message String contentType =
 * email.getContentType(); System.out.println(contentType);
 * 
 * if (contentType.toLowerCase().contains("multipart/mixed")) { // this message
 * must contain attachment Multipart multiPart = (Multipart) email.getContent();
 * 
 * for (int i = 0; i < multiPart.getCount(); i++) { MimeBodyPart part =
 * (MimeBodyPart) multiPart.getBodyPart(i); if
 * (Part.ATTACHMENT.equalsIgnoreCase(part.getDisposition())) {
 * System.out.println("Attached filename is:" + part.getFileName());
 * 
 * MimeBodyPart mimeBodyPart = (MimeBodyPart) part; String fileName =
 * mimeBodyPart.getFileName();
 * 
 * String destFilePath = System.getProperty("user.dir") + "\\Resources\\";
 * 
 * File fileToSave = new File(fileName); mimeBodyPart.saveFile(destFilePath +
 * fileToSave);
 * 
 * // download the pdf file in the resource folder to be read by PDFUTIL api.
 * 
 * PDFUtil pdfUtil = new PDFUtil(); String pdfContent =
 * pdfUtil.getText(destFilePath + fileToSave);
 * 
 * System.out.println("******---------------********");
 * System.out.println("\n");
 ...
 * return false; }
 * 
 * public String getEmailBody(Message email) throws IOException,
 * MessagingException {
 * 
 ...
 * Base64().decode(emailContentEncoded.toString().getBytes())); return
 * emailContentDecoded; }
 * 
